...
RSpec.describe AuthorizationsController, type: :controller do
  let(:auth_hash) { { provider: 'vk', uid: '123' } }
  let(:authorization_params) { { user: { email: 'user@server.com' } } }
  ...
  describe 'create' do
    ...
    context 'with auth data' do
      ...
      context 'with invalid email' do
        let(:invalid) { { user: { email: 'invalid.email' } } }

        it 'does not create user' do
          expect { post :create, params: invalid }.to_not change(User, :count)
        end
  ...
  describe 'email_confirmation' do
    ...
    let!(:authorization) { create(:authorization, user: user, uid: '123', provider: 'vk', confirmation_token: '88888888') }
    ...
    context 'with valid token' do
      ...
      it 'sign in user' do
        expect { get :email_confirmation, params: { confirmation_token: authorization.confirmation_token } }.to change(controller, :current_user).from(nil).to(user)
      end
    end
